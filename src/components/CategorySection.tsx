  ...
  return (
    <section className="container mx-auto px-4 py-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {categories.map((cat) => (
          <Link key={cat.title} to={`/category/${encodeURIComponent(cat.slug)}`} className="group bg-card rounded-xl border p-6 hover:shadow-xl hover:border-primary/20 hover:bg-primary/[0.02] transition-all duration-300">
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-4 ${cat.color} transition-all duration-300 group-hover:scale-110 group-hover:rotate-3`}>
