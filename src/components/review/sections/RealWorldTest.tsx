    ...
    <section className="bg-primary p-6 md:p-12 rounded-none md:rounded-[2.5rem] text-white space-y-8 md:space-y-12 overflow-hidden relative group">
      ...
      <div className="relative z-10 space-y-4">
        ...
        <p className="text-white/60 text-sm md:text-lg font-medium max-w-xl">
          เรานำอุปกรณ์ไปทดสอบจริงในสภาพแวดล้อมที่หลากหลาย เพื่อให้คุณเห็นประสิทธิภาพที่แท้จริง
        </p>
      </div>

      <div className="relative z-10 grid sm:grid-cols-3 gap-6 md:gap-8">
        {[
          { icon: MapPin, label: "Terrain Used", value: terrain },
          { icon: CloudRain, label: "Weather Condition", value: weather },
          { icon: Route, label: "Test Distance", value: distance }
        ].map((item, idx) => (
      ...
      {performance && (
        <div className="relative z-10 bg-accent/90 p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] shadow-2xl shadow-accent/20">
          <div className="flex items-center gap-4 mb-6">
            ...
            <h3 className="font-heading text-xl md:text-2xl font-bold text-white uppercase tracking-tight italic-prohibited">Performance Summary</h3>
          </div>
